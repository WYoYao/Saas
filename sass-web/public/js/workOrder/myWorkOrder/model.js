...
var myWorkOrderMethod = {//工单管理模块方法
    ...
    /*
     ...
     $('#controlSel').precover();
     myWorkOrderModel.customItem = {"name": "", "type": "", "items": [],"unit":""};
     ...
     var arr = [];
     for (var i = 0; i < myWorkOrderModel.infoArray.length; i++) {
     var item = myWorkOrderModel.infoArray[i];
     if (item.checked) {
     ...
     commonData.infoPoint_obj.obj_type = myWorkOrderModel.curObjType;
     ...
     //TODO: 验证
     myWorkOrderModel.customItem.name = $($(commonData.jqPopDataDivs2[4]).children()[0]).find('input').val();
     var obj = {
     name: myWorkOrderModel.customItem.name,
     type: myWorkOrderModel.customItem.type,
     unit: myWorkOrderModel.customItem.unit
     };

     if (commonData.controlName == '单选' || commonData.controlName == '多选') {
     var items = JSON.parse(JSON.stringify(myWorkOrderModel.customItem.items));
     var arr = [];
     ...
     myWorkOrderModel.workContent = JSON.parse(JSON.stringify(myWorkOrderModel.workContent));
     publicMethod.hideCurPop2();
     /!*setTimeout(function () {
     //选中对应的控件
     ...
     myWorkOrderModel.curObjType = 'custom'
     myWorkOrderMethod.setCurPop2(4, true);
     },
     //自定义信息点 弹框-添加选项
     addOption: function (event, contentIndex) {
     event.stopPropagation();
     commonData.contentIndex = contentIndex;
     myWorkOrderModel.customItem.items.push({name: ''});
     },
     */
    //------------------------------------------yn__end------------------------------------------
}
