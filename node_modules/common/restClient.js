...
function requestBefore() {
    ...
    this.fs = require('fs');
    this.requestTypes = { get: 'get', post: 'post', upload: 'upload', down: 'down', pdownloadByParam: 'pdownloadByParam' };
    this.request = new request();
    this.keyTool = require('./tool');
};
...
requestBefore.prototype = {
    ...
    plogin: function (req, res, next, requestType, objParam) {
        var _this = this;
        //objParam.fn = 'restUserService/login';
        startSendBefore.call(_this, req, requestType, objParam, null, objParam._c || function (err, result) {
            // startSendBefore.call(_this, req, requestType, objParam, null, objParam._c || function(err, result) {
            result = (result || [])[0] || {};
            result.p = objParam.data.user_psw || '';
            _this.responseTool.loginResponse(req, res, { err: err, puser: result });
        });
    },
    //获取数据
    pquery: function (req, res, next, requestType, objParam) {
        ...
        startSendBefore.call(_this, req, requestType, objParam, null, function (err, result, count) {
            ...
            var clientResult = _this.tool.parseResult(result, objParam.fn, null, count);
            if (!clientResult)
    ...
    pfiledownload: function (req, res, next, requestType, objParam) {
        var _this = this;
        var criteria = {};
        var _user = this.keyTool.getUserInfo(req);

        criteria.systemCode = _user._system_code || '';
        //criteria.secret = _user.image_secret || '';
        criteria.key = objParam.data.id;
    ...
    pupdate: function (req, res, next, requestType, objParam) {
        var _user = this.keyTool.getUserInfo(req);
        var systemCode = _user._system_code || '';
        var secret = _user._image_secret || '';
...
function createParam(req, objParam) {
    ...
    var project_id = _user.project_id || _user.last_project_id;
    ...
    if (userId) {
        if (param instanceof Array == true) {
            for (var i = 0; i < param.length; i++) {
                ...
                param[i].project_id = param[i].project_id ? param[i].project_id : project_id;
            }
        } else {
            param.user_id = userId;

            param.customer_id = param.customer_id ? param.customer_id : customer_id;
            param.project_id = param.project_id ? param.project_id : project_id;
        }
    }
...
function startSendBefore(req, requestType, objParam, url, call) {
    ...
    Object.keys(tempParam.criteria).forEach(function (key) {
        if (tempParam.criteria.hasOwnProperty(key))
            if (tempParam.criteria[key] == '--')
                tempParam.criteria[key] = '';
    })

    var par = {
        url: url || _config.serviceUrl,
...
request.prototype.upload = function (objParam) {
    ...
    var key = encodeURIComponent(criteria.key);

    var fileParamName = 'file';
    var realCriteria = {}
    realCriteria[fileParamName] =
        this.fs.readFileSync(criteria.path);
    //[this.fs.createReadStream(criteria.path)];
    var url = (criteria.fileType === 2 ? _config.fileUploadServiceUrl : _config.imgUploadServiceUrl) + 'systemId=' + criteria.systemCode + '&secret=' +
        criteria.secret + '&key=' + key;
    this.send.upload({
...
request.prototype.down = function (objParam) {
    var criteria = objParam.criteria || {};
    var fileType = criteria.fileType;
    var key = criteria.key;
    var url = fileType == 1 ? _config.imgDownServiceUrl + 'systemId=' + criteria.systemCode + '&key=' + encodeURIComponent(key) :
        _config.fileDownServiceUrl + key + '?' + 'systemId=' + criteria.systemCode;
...
send.prototype.parseResponse = function (responseErr, responseTxt, _httpInfo, isParse) {
    ...
    try {
        ...
        var returnObj = { err: errStr, data: content };
