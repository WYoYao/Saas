...
public class ProjectCache extends BaseService {
    ...
    public JSONObject getProjectJsonDataById(String projectId) throws Exception{
        if(projectCache.get(projectId) == null){
            synCache();
        }
        if(projectCache.get(projectId) == null){
        ...
        return projectCache.get(projectId).getJSONObject(CommonMessage.dataPlat_infos_dataName);
    }
